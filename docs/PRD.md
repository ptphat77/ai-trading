...
Automated Gold (XAU/USD) trading bot: uses technical indicators (MA, RSI, ATR) to collect market signals, then relies on **Gemini AI** to make the final decision on whether to enter a trade and where to set SL/TP. Trades via the **OANDA** broker, starting with a Demo account.
...
- Paper trading on OANDA Demo account
...
| # | Requirement |
|---|---|
| F1 | Fetch the last 100 M5 candles of XAU_USD from OANDA |
...
| F8 | Place Market Order with SL/TP via OANDA API |
...
- **Security**: API keys (OANDA, Gemini) are never committed to git.
...
|---|---|---|
| Leverage | 1:50 (OANDA standard) | Can be changed via config |
...
| Step | Content | Time |
|---|---|---|
| 1 | Setup project, OANDA client, test fetching candles | ~1h |
...
| OANDA API rate limit / downtime | Missed candles, failed order placement | Limited retries, log errors, don't hang state |
...
- `API-CONTRACTS.md` — API contracts (OANDA, Gemini, internal)
