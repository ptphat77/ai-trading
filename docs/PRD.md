...
Automated Gold (XAU/USD) trading bot: uses technical indicators (MA, RSI, ADX, ATR) to collect market signals, then relies on a **pluggable AI engine** (configurable via `AI_PROVIDER` in `.env`, default: Qwen) to make the final decision on whether to enter a trade and where to set SL/TP. Trades via a **configurable broker REST API**, starting with a Demo account.
...
**From operations**:
- 0% "blind trade" — no orders placed when AI provider times out or response parsing fails
...
| F3 | Aggregate indicators into context and send to active AI agent for a decision |
| F4 | Receive and validate JSON response from AI agent (action/confidence/sl/tp/reason) |
...
- **Security**: API keys (Broker, AI provider) are never committed to git.
- **Configurability**: All operational parameters (risk %, threshold, AI provider, AI model...) are read from `.env`, not hardcoded.
...
- **Extensibility**: Adding a new AI provider requires only a new `*Agent.js` file + one line in `AIAgentFactory` — no changes to bot logic.
...
| AI Provider | `AI_PROVIDER` in `.env` (`qwen` \| `gemini`) | Default: `qwen`. Switch at any time without code changes. |
...
| 2 | Indicators (MA, RSI, ADX, ATR) | ~30m |
| 3 | AI Agent layer: BaseAIAgent + QwenAgent + GeminiAgent + AIAgentFactory + promptTemplate | ~1.5h |
...
|---|---|---|
| AI returns malformed JSON | Bot might place wrong trade/crash | Strict validation in `BaseAIAgent._validateAndFormatDecision()` + fallback `skip` (see `PROJECT-RULES.md`) |
...
- `API-CONTRACTS.md` — API contracts (Broker REST API, AI providers, internal module interfaces)
